(() => {
  ...
  function imageMarkup(item) {
    if (item.image) {
      return `<img class="product-image" src="${esc(item.image)}" alt="${esc(item.brand + ' ' + item.name)}" loading="lazy" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"/><div class="image-placeholder" style="display:none">${esc(item.brand)}<br>${esc(item.name)}</div>`;
    }
  ...
  function card(item, sold=false) {
    ...
    const info = `
      <div class="product-info">
        ...
        <div class="meta">
          <span>${esc(details || item.category)}</span>
          <span class="price">${esc(item.price || '')}</span>
        </div>
        ${sold ? '' : '<span class="shop-arrow">Shop on TRR <span aria-hidden="true">↗</span></span>'}
      </div>`;
  ...
  function render() {
    ...
    soldGrid.innerHTML = sold.length ? sold.map(i => card(i, true)).join('') : '<p class="loading">No sold pieces yet.</p>';
  }
  ...
  fetch('/api/listings')
    .then(r => r.ok ? r.json() : Promise.reject(new Error('refresh unavailable')))
    .then(data => {
      if (!Array.isArray(data.listings) || !data.listings.length) return;
      const byUrl = new Map(fallback.map(x => [x.url, x]));
      listings = data.listings.map(live => {
        const saved = byUrl.get(live.url) || {};
        const cleanedLive = Object.fromEntries(Object.entries(live).filter(([,value]) => value !== '' && value !== null && value !== undefined));
        return {...saved, ...cleanedLive};
      });
      if (data.checkedAt) {
        const dt = new Date(data.checkedAt);
        updated.textContent = `Live TRR status · ${dt.toLocaleDateString(undefined,{month:'short',day:'numeric'})} ${dt.toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'})}`;
      }
      render();
    })
    .catch(() => {
      updated.textContent = 'Showing latest saved listing details';
    });
})();
